...
public:
    ...
    {
        // 订阅话题 名称
        std::string cloud_topic = this->declare_parameter<std::string>("cloud_topic", "/point_cloud");
        std::string odometry_topic = this->declare_parameter<std::string>("odometry_topic", "/odom");
        std::string output_topic = this->declare_parameter<std::string>("output_topic", "/cropped_cloud");
        output_topic_frame = this->declare_parameter<std::string>("output_topic_frame", "map");

        // 裁减的点云的长宽高
        double point_min_x = this->declare_parameter<double>("point_min_x", -5.0);
        double point_max_x = this->declare_parameter<double>("point_max_x", 5.0);
        double point_min_y = this->declare_parameter<double>("point_min_y", -5.0);
        double point_max_y = this->declare_parameter<double>("point_max_y", 5.0);
        double point_min_z = this->declare_parameter<double>("point_min_z", -7.0);
        ...
        // 2.5D 地图的相关参数
        planarVoxelSize = this->declare_parameter<double>("planar_voxel_size", 0.1); // 2.5D 地图的体素大小
        planarVoxelWidth = this->declare_parameter<int>("planar_voxel_width", 51); // 2.5D 地图整体的宽度
        maxObstacleHeight = this->declare_parameter<double>("max_obstacle_height", 1.0);// 最大障碍物高度
        maxGroundHeight = this->declare_parameter<double>("max_ground_height", 0.3);// 最大地面高度
    ...
    void task()
    {
        ...
        while (true)
        {
            ...
            if(pcl_task)
            {
                ...
                publisher_obstacle_->publish(ground_msg);
